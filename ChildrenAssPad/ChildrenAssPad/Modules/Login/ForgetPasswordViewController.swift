...
//    var accountTF: UITextField!
    ...
    func createUI() -> Void {
        ...
        tipL.text = localStringForKey(key: "forget_password")
        ...
        .asSingle()
            .subscribe(onSuccess: { [weak self] (mesg) in
                if let ws = self {
                    Prompter.hideIndicator(inView: ws.view)
                    ws.isCounting = true
                    Prompter.showTextToast(mesg, inView: ws.view)
                }
            }) { (error) in
    ...
    @objc func confirmBtnClicked() -> Void {
        print("确定按钮")
        LoginService.findPassword(phone: phoneNumTF.text ?? "", password: newPasswordTF.text ?? "", captcha: verificationTF.text ?? "")
            .catchError { [weak self] (error) -> Observable<String> in
                if let ws = self {
                    Prompter.showTextToast(APPErrorFactory.unboxAndExtractErrorMessage(from: error), inView: ws.view)
                }
